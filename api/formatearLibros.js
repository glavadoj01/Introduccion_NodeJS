...
async function formatearLibros(coleccionLibros) {
    ...
    */
	...
	// Contador para asignar IDs únicos si faltan (realmente, se reinicia en cada llamada a la función, pero es suficiente para este caso de un solo uso)
	let idFaltante = 1;
	...
	const librosFormateados = coleccionLibros.items.map(libro => {
		...
		return {
			...
			genero: volInfo.categories && volInfo.categories.length > 0 ? volInfo.categories : null,
			codigo_ISBN: codigoISBN, // Si no existen, genero uno siempre
			ISBN_Pr: isbnPr, // Siempre asignado como 1º elemento de codigo_ISBN
